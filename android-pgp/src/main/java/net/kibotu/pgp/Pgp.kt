...
object Pgp {
    ...
    fun decrypt(encrypted: ByteArray, password: String, compressed: Boolean): ByteArray? {
        ...
        if (pbe != null) {
            ...
            if (compressed) {
                ...
                val cData = pgpFact.nextObject() as PGPLiteralData
                val unc = cData.inputStream
                ByteArrayOutputStream().use {
                    val buffer = ByteArray(0xFFFF)
                    while (true) {
                        val r = unc.read(buffer)
    ...
    @JvmStatic
    @Throws(Exception::class)
    fun decrypt(encryptedText: String, password: String, compressed: Boolean): String? {
        return decrypt(encryptedText.toByteArray(), password, compressed)?.let { return String(it) }
    }
    ...
    fun encrypt(msg: ByteArray, compress: Boolean): ByteArray? {
        ...
        if(compress) {
            ...
            val lData = PGPLiteralDataGenerator()
            val pOut = lData.open(bOut, PGPLiteralData.BINARY, PGPLiteralData.CONSOLE, msg.size.toLong(), Date())
            pOut.write(msg)
            lData.close()
        }
    ...
    @JvmStatic
    @Throws(IOException::class, PGPException::class)
    fun encrypt(msgText: String, compress: Boolean): String? {
        return encrypt(msgText.toByteArray(), compress)?.let { return String(it) }
    }
