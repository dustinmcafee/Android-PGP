...
class EncryptionTest : BaseTest() {
    ...
    fun encryptionProvidedTest() {
        ...
        assertThat(Pgp.decrypt(encryptedMessage, password, true)).isEqualTo(decryptedMessage)

        assertThat(Pgp.encrypt(decryptedMessage, true)).isNotEqualTo(encryptedMessage)

        // decrypting multiple encrypted different messages should always be the same decrypted
        (0 until 100).forEach {
            assertThat(Pgp.decrypt(Pgp.encrypt(decryptedMessage, true)!!, password, true)).isEqualTo(decryptedMessage)
        }
    }
    ...
    fun encryptionStringTest() {
        ...
        val encrypt = Pgp.encrypt(expected, true)
        val decrypt = Pgp.decrypt(encrypt!!, password, true)
